...
export const ACCEPTANCE_CASES = {
  ...
  'EVM-IDEM-001': { description: 'a cached result is released only to the settled payment authorization; an identifier-only or differently-authorized request is refused without settlement', scope: 'local' },
  'EVM-IDEM-002': { description: 'overlapping requests under one payment identifier share one operation: one verification, one execution, one settlement', scope: 'local' },
  'EVM-IDEM-003': { description: 'a rejected attempt releases the identifier for a corrected retry; an uncertain settlement outcome refuses retries', scope: 'local' },
  'EVM-IDEM-004': { description: 'the identifier store refuses new identifiers at capacity and never evicts', scope: 'local' },
  ...
  'EVM-TAMPER-002': { description: 'a tampered observed field value fails its own digest and its native decode, and nothing bound beside it', scope: 'local' },
  'EVM-TAMPER-003': { description: 'an edited record payload is refused with an invalid signature', scope: 'local' },
  'EVM-TAMPER-004': { description: 'a valid native artifact with an altered observation fails the observation and its agreement with the artifact, never the artifact digest', scope: 'local' },
  'EVM-TAMPER-005': { description: 'a removed artifact fails both its digest and the presence contract', scope: 'local' },
  'EVM-NATIVE-001': { description: 'a re-signed native payment-signature whose amount disagrees with the record fails the named terms and authorization checks', scope: 'local' },
  'EVM-NATIVE-002': { description: 'a re-signed native payment-response whose transaction disagrees with the observation fails the named settlement check', scope: 'local' },
  'EVM-NATIVE-003': { description: 'a re-signed native payment-signature naming a different resource fails the request-binding agreement check', scope: 'local' },
  'EVM-NATIVE-004': { description: 'a payment identifier that disagrees with the record reference fails by name', scope: 'local' },
  'EVM-NATIVE-005': { description: 'evidence of a rejected payment attempt with a malformed native artifact stays verifiable under the preserved rule', scope: 'local' },
  'EVM-NATIVE-006': { description: 'the chain observation document is held to its closed schema; an unknown member fails by name', scope: 'local' },
  'EVM-NATIVE-007': { description: 'the archived live evidence and the committed fixture verify under profile 2 with every native check passing', scope: 'local' },
